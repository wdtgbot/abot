...
def run_rclonecopy(onedir,twodir,client, message):
    ...
    shell=f"rclone copy {onedir} {twodir}  -v --stats-one-line --stats=3s --log-file=\"{name}.log\" --transfers=8 --cache-chunk-size=20M --ignore-existing -u --ignore-errors --check-first --checkers=8 "
    print(shell)
    sys.stdout.flush()
    try:
