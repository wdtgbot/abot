...
def run_rclonecopy(onedir,twodir,client, message):
    ...
    shell=f"rclone copy {onedir} {twodir}  -v --stats-one-line --stats=3s --log-file=\"{name}.log\" --transfers=8 --cache-chunk-size=20M "
    print(shell)
    sys.stdout.flush()
    try:
        ...
        with open(fname, 'r') as f:  #打开文件
            try:
                ...
                if temp_text != last_line and "ETA" in last_line:
                    print(last_line)
                    sys.stdout.flush()
                    log_time,file_part,upload_Progress,upload_speed,part_time=re.findall(".+?ETA (.+?)s",last_line , re.S)[0]
                    text=f"源地址:`{onedir}`\n" \
                         f"目标地址:`{twodir}`\n" \
                         f"更新时间：`{log_time}`\n" \
                     f"传输部分：`{file_part}`\n" \
