...
def run_rclonecopy(onedir,twodir,client, message):
    ...
    shell=f"rclone copy {onedir} {twodir}  -v --stats-one-line --stats=3s --log-file=\"{name}.log\" --transfers=8 --cache-chunk-size=20M --ignore-checksum --no-update-modtime --checkers=8 "
    print(shell)
    sys.stdout.flush()
    try:
